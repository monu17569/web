...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
  if (isset( $_POST['idEdit'])){
    // Update the record
  $id = $_POST['idEdit'];
  $journal = $_POST["journalEdit"];
  $indexed = $_POST["indexedEdit"];
  $publisher = $_POST["publisherEdit"];
  

// Sql query to be executed
  $sql = "UPDATE `reviewer` SET `journal` = '$journal' , `indexed` = '$indexed' , `publisher`  = '$publisher' WHERE `reviewer`.`id` = $id";
  $result = mysqli_query($conn, $sql);
  if($result){
    $update = true;
  }
...
<div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
                ...
                <form action="./awards.php" method="post">
                  <div class="modal-body">
                    ...
                    <div class="mb-3">
                      ...
                      <input type="text" class="form-control" id="journalEdit" name="journalEdit" required>
                    </div>

                    <div class="mb-3">
                      <label for="indexed" class="form-label"> <h5>Indexed</h5></label>
                      <input type="text" class="form-control" id="indexedEdit" name="indexedEdit" required >
                    </div>

                    <div class="mb-3">
                      <label for="publisher" class="form-label"> <h5>Publisher</h5></label>
                      <input type="text" class="form-control" id="publisherEdit" name="publisherEdit" required >
                    </div>
      ...
      <?php if($delete){
        ...
        <a href="./newReviewer.php"><button type="button" class="btn btn-warning"><h4>Add New</h4></button></a>
        </div>
   <div class="container my-5" id="reviewer">
  ...
  <script>
       ...
       journal = tr.getElementsByTagName("td")[0].innerText;
       indexed = tr.getElementsByTagName("td")[1].innerText;
       publisher = tr.getElementsByTagName("td")[2].innerText;
       console.log(journal, indexed , publisher);
       journalEdit.value = journal;
       inxededEdit.value = inxeded;
    ...
    Array.from(deletes).forEach((element) => {
        ...
        if (confirm("Are you sure you want to delete !")) {
          console.log("yes");
          window.location = `./reviewer.php?delete=${id}`;
        }
        else {
          console.log("no");
        }
