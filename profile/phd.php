...
if(isset($_GET['delete'])){
  $id = $_GET['delete'];
  $delete = true;
  $sql = "DELETE FROM `phd` WHERE `id` = $id";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
  if (isset( $_POST['idEdit'])){
    ...
    // Sql query to be executed
      $sql = "UPDATE `phd` SET `names` = '$name' ,`title` = '$title' ,`stat` = '$status'  WHERE `phd`.`id` = $id";
      $result = mysqli_query($conn, $sql);
      if($result){
        $update = true;
      }
...
<body>
     ...
     <a href="./newPhd.php"><button type="button" class="btn btn-warning"><h4>Add New</h4></button></a>
     </div>

   <div class="container my-5" id="phd" >
    ...
    Array.from(deletes).forEach((element) => {
      element.addEventListener("click", (e) => {
        ...
        if (confirm("Are you sure you want to delete !")) {
          console.log("yes");
          window.location = `./phd.php?delete=${id}`;
        }
        else {
          console.log("no");
        }
