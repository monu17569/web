...
// Sql query to be executed
  $sql = "UPDATE `conference` SET `Disc` = '$description', `years` = '$year'  WHERE `conference`.`id` = $id";
  $result = mysqli_query($conn, $sql);
  if($result){
    $update = true;
  }
...
<body>
        ...
        <a href="./newConference.php"><button type="button" class="btn btn-warning"><h4>Add New</h4></button></a>
        </div>
    ...
    Array.from(deletes).forEach((element) => {
      element.addEventListener("click", (e) => {
        ...
        if (confirm("Are you sure you want to delete !")) {
          console.log("yes");
          window.location = `./conference.php?delete=${id}`;
        }
        else {
          console.log("no");
        }
