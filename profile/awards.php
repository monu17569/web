...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
  if (isset( $_POST['idEdit'])){
    // Update the record
  $id = $_POST['idEdit'];
  $awardName = $_POST["awardNameEdit"];
  $description = $_POST["descriptionEdit"];
  

// Sql query to be executed
  $sql = "UPDATE `awards` SET `Disc` = '$description' , `awardName` = '$awardName' WHERE `awards`.`id` = $id";
  $result = mysqli_query($conn, $sql);
  if($result){
    $update = true;
  }
...
<div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
              ...
              <div class="modal-content">
                ...
                <form action="./awards.php" method="post">
                  <div class="modal-body">
                    ...
                    <div class="mb-3">
                      <label for="awardname" class="form-label"> <h5>Award Name</h5></label>
                      <input type="text" class="form-control" id="awardNameEdit" name="awardNameEdit" required>
                    </div>
  ...
  <script>
       ...
       awardname = tr.getElementsByTagName("td")[1].innerText;
       console.log(description, awardname);
       descriptionEdit.value = description;
       awardNameEdit.value = awardname;
    ...
    Array.from(deletes).forEach((element) => {
        ...
        if (confirm("Are you sure you want to delete !")) {
          console.log("yes");
          window.location = `./awards.php?delete=${id}`;
        }
        else {
          console.log("no");
        }
