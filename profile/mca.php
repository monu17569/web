...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
  if (isset( $_POST['idEdit'])){
    ...
    // Sql query to be executed
      $sql = "UPDATE `mca` SET `names` = '$name' , `rollno` = '$rollno',`title` = '$title' ,`duration` = '$duration'  WHERE `mca`.`id` = $id";
      $result = mysqli_query($conn, $sql);
      if($result){
        $update = true;
      }
...
<body>
     ...
     <a href="./newMca.php"><button type="button" class="btn btn-warning"><h4>Add New</h4></button></a>
     </div>

   <div class="container my-5" id="mca" >
  ...
  <script>
   ...
   Array.from(edits).forEach((element) =>{
     element.addEventListener("click",(e) =>{
       ...
       console.log(name, rollno, title, duration);
       nameEdit.value = name;
    ...
    Array.from(deletes).forEach((element) => {
      element.addEventListener("click", (e) => {
        ...
        if (confirm("Are you sure you want to delete !")) {
          console.log("yes");
          window.location = `./mca.php?delete=${id}`;
        }
        else {
          console.log("no");
        }
